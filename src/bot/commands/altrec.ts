...
export const data = new SlashCommandBuilder()
	.setName("altrec")
	.setDescription(
		"Recommendations from the alternative leaderboard (3-9 ratings)",
	)
...
export async function execute(
	...
	try {
		...
		>(
            ...
            HAVING COUNT(r.rating) BETWEEN 3 AND 9
		...
		if (recommendations.length === 0) {
			await interaction.editReply({
				content: "No unseen films from the alternative leaderboard!",
			});
			return;
		}

		const embed = createFilmLeaderboardEmbed(
			"🎯 Alternative Recommendations (3-9 ratings)",
			...
			1,
			{
				description: `Unseen films from the alternative leaderboard\nOne more rating and they'll join the main leaderboard!`,
			},
		);
